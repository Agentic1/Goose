...
fn session_log_path(sid: &str) -> PathBuf {
    ...
    let home_dir = "/home/ag1azure".to_string();
...
impl GooseSession {
    /// Send user input to the Goose CLI process
    pub async fn send_user(&mut self, text: &str) -> Result<()> {
        ...
        stdin.write_all(b"\n").await
            .map_err(|e| anyhow!("Failed to write newline to stdin: {}", e))?;
    ...
    pub async fn start(cfg: &Config, sid: String) -> Result<Self> {
        ...
        // Explicitly add the AG1 MCP server extension
        let mcp_server_path = "/Users/admin/code/cargo-target/debug/ag1_mcp_server";
        debug!(path = %mcp_server_path, "Adding AG1 MCP server extension");
        cmd.arg("--with-extension").arg(mcp_server_path);
        
        // Set environment variables that might be needed by the MCP server
    ...
    /// Update the last file offset for this session
    ...
    pub async fn wait_assistant_jsonl(
        ...
        let mut reader = FramedRead::new(
            file,
            LinesCodec::new_with_max_length(10 * 1024 * 1024),
        );
        let mut last_file_size = start_offset;

        let mut buffer = String::new();
        let mut record_offset = current_offset;
        ...
        loop {
            ...
            match tokio_timeout(
                ...
                Ok(Some(Ok(line))) => {
                    ...
                    // one-object‑per‑line JSONL ⇒ parse each line in isolation
                    match serde_json::from_str::<serde_json::Value>(line) {
                        Ok(json) if json.get("role").and_then(|r| r.as_str()) == Some("assistant") => {
                        ...
                        Ok(_) => {
                            // it was valid JSON, just not an "assistant" record
                        }
                        Err(e) => {
                            debug!(
                                session_id = %self.sid,
