...
pub async fn create_dynamic_task(params: Value, tasks_manager: &TasksManager) -> ToolCallResult {
    // Initialize bus with Redis server
    let bus = match Bus::new("redis://admin:UltraSecretRoot123@forge.agentic1.xyz:8081") {
        Ok(bus) => bus,
        Err(e) => {
            log::error!("Failed to initialize bus: {}", e);
            return ToolCallResult::from(Err(ToolError::ExecutionError(
                format!("Failed to initialize message bus: {}", e)
            )));
        }
    };
    ...
    // Get task IDs for notification
    let task_ids: Vec<String> = tasks.iter().map(|t| t.id.clone()).collect();
    
    // Create and send notification
    let envelope = create_envelope(
        serde_json::json!({
            "text": "Dynamic tasks created successfully",
            "task_ids": task_ids
        }),
        "system",
        None
    ...
    if let Err(e) = bus.send("tasks", &envelope).await {
        log::error!("Failed to send task completion notification: {}", e);
    }
    
    // Return the result
    match serde_json::to_string(&task_execution_payload) {
        Ok(json) => ToolCallResult::from(Ok(vec![Content::text(json)])),
        Err(e) => ToolCallResult::from(Err(ToolError::ExecutionError(format!(
