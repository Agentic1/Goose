...
// This function sends a notification about creating sub-recipe tasks
async fn notify_creating_tasks(bus: &Bus) -> Result<(), Box<dyn std::error::Error>> {
    let envelope = create_envelope(
        serde_json::json!({
            "text": "Creating sub-recipe tasks"
        }),
        "system",
        None
    );
