...
async fn notify_task_completion(bus: &Bus, task_id: &str) -> Result<(), Box<dyn std::error::Error>> {
...
// Helper function to notify about task execution start with parameters
async fn notify_task_execution_start(bus: &Bus, execute_data: &Value) -> Result<(), Box<dyn std::error::Error>> {
    let envelope = create_envelope(
        serde_json::json!({
            "text": "Executing tasks with parameters: ",
            "params": execute_data
        }),
        "system",
        None
