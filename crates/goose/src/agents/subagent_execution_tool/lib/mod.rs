use ag1_meta::create_envelope;
use bus::Bus;
use anyhow::Result;

// Helper function to notify about task execution errors
async fn notify_execution_errors(bus: &Bus, failed_tasks: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    let error_env = create_envelope(
        serde_json::json!({
            "text": "Task execution encountered errors",
            "failed_tasks": failed_tasks
        }),
        "system",
        None
    );
    bus.send("tasks", &error_env).await?;
    Ok(())
}

// Helper function to notify about successful task completion
async fn notify_execution_completion(bus: &Bus) -> Result<(), Box<dyn std::error::Error>> {
    let completed_env = create_envelope(
        serde_json::json!({"text": "Tasks executed successfully"}),
        "system",
        None
    );
    bus.send("tasks", &completed_env).await?;
    Ok(())
}

// Helper function to notify about starting task execution
async fn notify_execution_start(bus: &Bus) -> Result<(), Box<dyn std::error::Error>> {
    let start_envelope = create_envelope(
        serde_json::json!({"text": "Starting execution of tasks"}),
        "system",
        None
    );
    bus.send("tasks", &start_envelope).await?;
    Ok(())
}
