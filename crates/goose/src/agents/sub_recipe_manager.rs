...
async fn notify_task_completion(bus: &Bus, task_ids: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    let task_completed_env = create_envelope(
        serde_json::json!({
            "text": "Sub-recipe task executed successfully",
            "task_ids": task_ids
        }),
        "system",
        None
    );
    bus.send("tasks", &task_completed_env).await?;
    Ok(())
}

// Helper function to notify about dispatching tool calls
async fn notify_dispatching_tool(bus: &Bus) -> Result<(), Box<dyn std::error::Error>> {
    let envelope = create_envelope(
        serde_json::json!({
            "text": "Dispatching sub-recipe tool call"
        }),
        "system",
        None
    );
    bus.send("tasks", &envelope).await?;
    Ok(())
}
