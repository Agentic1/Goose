...
ENVELOPE_SIZE_LIMIT = 1024 * 1024 * 1024  # 128 KB
...
def get_target_agent_from_channel(channel: str) -> str:
    """Return the full channel path as the target agent identifier.
    
    This preserves the complete routing information and avoids any parsing ambiguity.
    The full path provides better traceability and works with any channel format.
    """
    if not channel:
        return 'unknown'
        
    # Remove any stream group suffix (e.g., '>', '0-0') but keep the full path
    clean_channel = channel.split('>')[0].strip()
    print(f'[Bus][Target_channel] {clean_channel}')
    # Return the full channel path
    return clean_channel if clean_channel else 'unknown'

# --- Envelope Publisher (with Discovery + Size Guard) ---
async def publish_envelope(redis, channel: str, env: Envelope, meter_traffic: bool = True):
    ...
    try:
        # Define data_to_publish *before* it's used by the metering logic.
        data_to_publish = json.dumps(env.to_dict())
    except Exception as err:
        print(f"[BUS][ERROR] envelope serialization failed: {err}")
        raise

    # --- START: METERING LOGIC ---
    if meter_traffic:
        try:
            target_agent = get_target_agent_from_channel(channel)
            
            payload_preview = None
            is_encrypted = env.meta.get("is_encrypted", False) 
            
            if not is_encrypted and isinstance(env.content, dict):
                preview_str = json.dumps(env.content)
                payload_preview = preview_str[:256]
            
            # Create the CDR envelope
            usage_event_envelope = Envelope(
                role="metering_event",
                agent_name="AetherBus", # The bus itself is the source of the CDR
                # --- THIS IS THE PAYLOAD FOR THE BILLING AGENT ---
                content={
                    "source_agent": env.agent_name,
                    "target_agent": target_agent,
                    "size_bytes": len(data_to_publish.encode('utf-8')),
                    "is_encrypted": is_encrypted,
                    "payload_preview": payload_preview,
                    
                    # --- THE CRITICAL FIX ---
                    # Add the destination channel key to the content.
                    # This is what the BillingAgent needs for infrastructure-based pricing.
                    "destination_channel": channel
                },
                correlation_id=env.correlation_id,
                # Pass through original metadata if needed
                meta=env.meta 
            )
            
            # Publish to the dedicated billing ledger stream
            billing_stream = key_builder.billing_ledger()
            # The BillingAgent's `subscribe` function expects the field name to be 'data'
            await redis.xadd(billing_stream, {"data": json.dumps(usage_event_envelope.to_dict())})

        except Exception as e:
            # CRITICAL: If metering fails, we must NOT stop the original message.
            print(f"[BUS][METERING_ERROR] Failed to publish usage event: {e}")
            traceback.print_exc()
    # --- END: METERING LOGIC ---
        ...
        try:
            ...
            for stream_name_bytes, messages_in_stream in response:
                for message_id_bytes, message_data_dict_bytes in messages_in_stream:
                    ...
                    # --- START OF FIX BILLING---
                    # The message ID can be bytes or string depending on redis client config.
                    if isinstance(message_id_bytes, bytes):
                        current_message_id = message_id_bytes.decode('utf-8')
                    else:
                        current_message_id = message_id_bytes
                    # --- END OF FIX ---
